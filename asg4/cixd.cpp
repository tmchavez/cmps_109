...
void reply_rm(accepted_socket& client_sock, cix_header& header){
   int suc = unlink(header.filename);
   if(suc == -1){
     log << "rm unsuccsesful" << strerror (errno) << endl;
     header.command = cix_command::NAK;
     header.nbytes = errno;
     send_packet (client_sock, &header, sizeof header);
     return;
...
void reply_get(accepted_socket& client_sock, cix_header& header){
   ...
   string outs = "";
   if(not copyfile.good()){
     log << header.filename << ": file not found" << endl;
     header.command = cix_command::NAK;
     send_packet (client_sock, &header, sizeof header);
     return;
   }
   ...
   log << "sending header " << header << endl;
   send_packet (client_sock, &header, sizeof header);
   send_packet (client_sock, outs.c_str(), outs.size());
   log << "sent " << outs.size() << " bytes" << endl;
} 
